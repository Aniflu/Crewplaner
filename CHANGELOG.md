...
- **Hook v4.11:** beide `sendMail`-Funktionen mit Guard „kein `RESEND_KEY` → Mailversand überspringen" (Test-Backend sendet keine echten Mails); Anleitungs-Links `aniflu.github.io` → `crewplanner.nyxlightwork.de/docs`.
- **Rollout (2026-07-30, mit Admin):** Test-PB live, Live-Frontend auf `live`-Branch entkoppelt, Hook **v4.11 auf BEIDEN** Backends, v0.31.0 produktiv auf beiden Frontends, `aniflu.github.io` aus der Live-CORS entfernt (Testseite sauber von der Live-DB isoliert). Admin-Runbooks: `docs/admin-runbook-umzug.md`, `docs/admin-runbook-golive.md`, `docs/admin-runbook-pb-upgrade.md`.
- **Offen (optional, geplant):** PocketBase-Versions-Upgrade beider Instanzen (Live läuft noch auf altem Mai-Image) — mit Backup + Test-first, siehe `docs/admin-runbook-pb-upgrade.md`.
