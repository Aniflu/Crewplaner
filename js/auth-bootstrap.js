...
(async function authBootstrapFlow() {
  ...
  if (!token || !userStr) {
    if (!EXEMPT_PAGES.includes(currentPage)) {
      ...
      window.location.replace(window.getNavUrl('login.html'));
      return;
    }
  ...
  try {
    ...
    // Prüfe SOWOHL Flag als auch tatsächliche Daten (robuster)
    const hasPlanTransfer = !!localStorage.getItem('_planTransfer_flag') || !!localStorage.getItem('_planTransfer_data');
    ...
    if (currentPage === 'admin.html' && !isAdmin) {
      ...
      window.location.replace(window.getNavUrl('index.html'));
      return;
    }
