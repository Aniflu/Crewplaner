...
function renderHead(){
  let h='<tr>';
  h+='<th rowspan="2" style="left:0;text-align:left;vertical-align:middle;z-index:12;">Datum</th>';
  ...
  POSITIONS.forEach(p=>{
    ...
    h+=`<th style="background:#12141a;border:1px solid var(--border);border-top:none;padding:3px;">
      ...
      ${def?`<button onclick="requestForPos(event,'${p.id}')" style="margin-top:3px;width:100%;background:rgba(74,232,160,.1);border:1px solid rgba(74,232,160,.3);color:#4ae8a0;padding:2px 5px;font-family:'IBM Plex Mono',monospace;font-size:.58rem;border-radius:3px;cursor:pointer;">↗ Anfragen</button>`:''}
