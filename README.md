...
**v0.26.0** — fix/chore: **Code-Review-Härtung & Aufräumen (kein sichtbares Feature).** Ergebnisse eines Gesamt-Reviews: ungenutzte Alt-Dateien (`bundle.js`, `userView.test.js`) entfernt; die Dropdown-Anzeige escaped jetzt Namen konsequent (XSS-Härtung, konsistent mit v0.23.3); ein PocketBase-Filter-Escape und ein fehlender Null-Schutz beim Hinweis-Toast repariert; nach einem Tour-/Plan-Wechsel springt die Tabelle wieder einmalig zur „Heute"-Zeile. Alles rein clientseitig, 85 Tests grün. *(Server-seitige Zugriffsregeln bleiben ein bewusst separater, getesteter Schritt.)*
