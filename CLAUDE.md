...
```
v0.50.0 — feat(Sammel-Statuswechsel „bestätigt ⇄ vorgemerkt" + Status im Kalender): User-Report: „das vorgemerkt ist jetzt sau umständlich auf alle Personen zu münzen. ich brauche die möglichkeit personen die zugesagt haben, am stück, innerhalb einer tour, auf vorgemerkt zu setzen." Der v0.29.0-Status ✎ ließ sich nur ZELLE FÜR ZELLE setzen — bei 30–60 Tourtagen × mehreren Positionen unbrauchbar; für „✓ bestätigen" gab es die Sammelaktion längst (dropdown.js:122 „✓ Alle Termine von X bestätigen"). Superpowers-Brainstorming klärte den Umfang (Spec: docs/superpowers/specs/2026-07-31-bulk-status-vormerken-design.md): Auswahl Person → Tourblock → Tag mit Checkboxen je Ebene; Quelle nur `confirmed`; BEIDE Richtungen per Umschalter; Einstieg Sidebar UND Zellen-Menü (Person vorausgewählt); Benachrichtigung über die bestehende Update-Queue (Versand wie immer per Knopfdruck, nie automatisch). ZWEI ANFORDERUNGEN GINGEN ÜBER DEN DIALOG HINAUS (User: „die termine bleiben natürlich im kalender, kannst gern im infofeld einen status hinzufügen der dann vorgemerkt oder wenn ich sie bestätige auf status bestätigt geändert wird"): (a) KALENDER — bis dato filterten SOWOHL der abonnierbare Server-Feed ALS AUCH der Client-Export strikt auf `confirmed`, eine Vormerkung hätte den Termin also LAUTLOS aus dem Kalender der Person verschwinden lassen. (b) die Crew soll den Statuswechsel per Update-Mail erfahren. UMSETZUNG: (1) NEU `js/bulkStatus.js` (eigenes Modul, userView.js ist mit ~900 Zeilen groß genug): `openBulkStatusModal(prefillCrew)`, Modus `pencil`/`confirm` bestimmt nur den Quellstatus; sammelt TOUR_DATES×POSITIONS nach `assignmentStatuses[date][pos].status`, gruppiert Person → Block (via `_dateBlockId`, dafür in userView.js exportiert) → Tag; Auswahl-Muster („alle/keine" je Block und je Person, globales ALLE/KEINE) bewusst 1:1 wie das Update-Queue-Modal, Auswahlzustand aber NUR modul-lokal (Einmal-Aktion, nichts zu persistieren); Anwenden ruft die BESTEHENDEN `pencilInAssignment`/`confirmAssignment` + `_queueStatusSlot`, bei Fehler Toast mit Teilerfolg-Zähler + `loadAssignmentStatuses()`-Resync. (2) QUEUE: dritte Sorte `kind:'status'` mit `to:'pencilled'|'confirmed'` — EINE Sorte für beide Richtungen, damit Modal/Mail/Tests nur einen Zweig kennen; `_queueStatusSlot` aktualisiert bei erneutem Umschalten desselben Slots nur die Richtung (sonst stünden nach Hin-und-Zurück zwei widersprüchliche Einträge in der Mail); Self-Heal-Regel `getVal(date,posId)===name` (anders als removed sind status-Slots weiter in getVal — nimmt der Manager die Person ganz raus, fällt der Eintrag weg und der Entfernen-Pfad legt seinen eigenen removed-Eintrag an); Modal-Tag „✎ vorgemerkt"/„✓ wieder bestätigt"; Vorschau-Popup bekam eine Art-Spalte + eigenen Einleitungstext für reine Statuswechsel („bitte erneut bestätigen" wäre dort falsch). ⚠️ KERN-FALLE (der teuerste denkbare Fehler, testabgesichert): `_sendUpdateForEntry` musste status-Slots — wie removed — aus `normal` ausschließen, sonst patcht der „Änderung an bestehenden Einsätzen"-Zweig sie auf `proposed`, zerstört die eben gesetzte Vormerkung UND löst eine Anfrage-Mail aus. (3) KALENDER: `confirmedIcsRows`→**`icsExportRows`** umbenannt (der alte Name log jetzt), nimmt `pencilled` mit auf und liefert pro Tag den höchsten Rang (`confirmed > pencilled`; declined/cancelled/cancel_acked bleiben bewusst draußen) sowie `status` pro Slot; neue Konstante `ICS_STATUS_LABEL` (Bestätigt/Angefragt/Vorgemerkt) EINMAL in pure.js, damit Download und Server-Feed denselben Wortlaut haben; `crewIcsContent` + `calendar.js` schreiben `Status: …` in die DESCRIPTION und setzen VEVENT `STATUS:CONFIRMED` nur bei bestätigt (sonst TENTATIVE); Crew-PDF bekam eine Status-Spalte. (4) HOOK v4.12: /ics-Route nimmt `pencilled` mit auf, Tagesstatus über eine RANK-Tabelle (confirmed>proposed>pencilled) statt des bisherigen bool-Flags, `Status:`-Zeile in der DESCRIPTION — die stabile UID (`planId-date@crewplanner`) bleibt, Kalender-Apps ERSETZEN den Termin also beim nächsten Abruf und der Status wandert ohne Dublette mit; Update-Mail bekam zwei neue Abschnitte („✎ Jetzt vorgemerkt — vorläufig geplant, noch nicht verbindlich. Du musst nichts tun." / „✓ Wieder verbindlich bestätigt"), bewusst OHNE Aktions-Button (hier ist nichts zu bestätigen); rückwärtskompatibel (unter v4.11 landen die Slots im generischen ➕-Abschnitt). (5) `calcByPers` (stats.js) BEWUSST unverändert — vorgemerkt zählt weiterhin NICHT als bestätigter Tag, damit die KPI ehrlich bleibt. TESTS: pure (vorgemerkt im Export mit `Status: Vorgemerkt`, confirmed schlägt pencilled, declined/cancelled draußen, VEVENT-STATUS), queue (`_queueStatusSlot` legt kind/to an, Self-Heal in beide Richtungen, **`_sendPendingUpdates` patcht status-Slots NICHT auf proposed** — fetch-gemockt). 116 grün (TZ UTC+Berlin). VERIFIZIERT mit ECHT AUSGEFÜHRTEM Code in Headless-Chrome (echtes Modal-Markup aus index.html, echte state/bulkStatus/userView-Module, echtes theme.css/styles.css): Dialog öffnet gruppiert nach Person→Block→Tag, „alle" für eine Person → 5 ausgewählt, „keine" für einen Block → 3, Anwenden patcht GENAU die 3 (der abgewählte Block bleibt confirmed), Status lokal auf pencilled, Queue enthält 3× {kind:'status',to:'pencilled'}, Updates-Badge springt auf 1, Rückweg-Modus listet danach korrekt die vorgemerkten. NEBENFUND: der Import-Guard (tests/imports.test.mjs) meldete einen FALSE POSITIVE — sein Template-Literal-Stripping kommt bei VERSCHACHTELTEN Template-Literalen (`${x ? `…${y}…` : ''}`) aus dem Tritt und lässt danach HTML-Attribute wie `data-crew=` als Code stehen. Im Modul aufgelöst (kein verschachteltes Literal mehr); der Guard selbst hat die Lücke weiterhin (auch als potenzieller False NEGATIVE) — Backlog. app.js?v=44→45. BACKEND-SCHRITT (Admin): Hook v4.12 auf BEIDE Backends deployen (erst Test, dann Live) — bis dahin läuft der App-Teil vollständig, nur Mail-Wortlaut und Kalender-Status hinken hinterher.
v0.31.0 — feat(Zwei getrennte Umgebungen: GitHub = Test, Hetzner = Live): User-Wunsch — Produktiv soll vollständig auf dem Hetzner-Server laufen (keine GitHub-Datei im Livebetrieb), und die GitHub-Seite soll als ISOLIERTE Testumgebung dienen (Bugfixes/Features erst testen, dann bewusst „Go-Live"). Superpowers-Brainstorming klärte: (a) Admin bestätigte, Live läuft bereits in einem eigenen Coolify-Container (od48m2ubvy7rqq55fofbqgph-154940502903, eigenes nginx-Image; GitHub ist nur Bauquelle → runtime-seitig ist „weg von GitHub" schon erfüllt, KEIN neuer Container nötig). (b) Bisher redeten Test (GitHub Pages) UND Live mit DERSELBEN Live-PocketBase → Testaktionen wirkten auf echte Daten/Mails. Entscheidung: eigenes Test-Backend + Branch-Trennung (`main`=Test, `live`=Produktion, Go-Live = `main`→`live` pushen, self-service ohne Admin/SSH). KERN-CODE-UMBAU (der einzige): Frontend wählt die PocketBase-API automatisch nach `location.hostname` statt sie fest zu verdrahten — neue reine `pickApiUrl(host)` in pure.js (Sicherheits-Default „im Zweifel Test": NUR `crewplanner.nyxlightwork.de`/`www.` → Live-API `api.crewplanner…`; GitHub Pages/localhost/unbekannt → Test-API `api-test.crewplanner.nyxlightwork.de`, damit nie versehentlich in die Live-DB geschrieben wird). Ersetzt 5 hardcodierte Stellen: js/config.js (importiert pickApiUrl, Fallback-Kette window.POCKETBASE_URL → location.hostname → Live für Node/Tests), die inline `<head>`-Skripte in index/login/admin.html (inline-Duplikat wie getNavUrl), js/pb-login-bundle.js (übernimmt window.POCKETBASE_URL). view.html braucht keine Kopfzeile (läuft über config.js). HOOK v4.11 (main.pb.js): (1) BEIDE `sendMail`-Funktionen mit Guard „kein `RESEND_KEY` → Mailversand still überspringen" → das Test-Backend (bewusst ohne Key) verschickt garantiert keine echten Mails, ohne 401-Calls/Crash. (2) Anleitungs-Links (crew/admin-Guide) von `aniflu.github.io/Crewplaner/docs/` auf `crewplanner.nyxlightwork.de/docs/` umgebogen (docs liegen dort, 200 geprüft) → letzter GitHub-Verweis aus dem Live-Mail-Weg raus. ADMIN-ANLEITUNG (docs/umzug-zwei-umgebungen.md, da Marco kein SSH): Test-PocketBase aufsetzen (Subdomain api-test, CORS aniflu.github.io, KEIN RESEND_KEY, Schema alle-text, v0.26.0-Regeln, feed_token, Hook v4.11, Superuser), Live-Coolify-App auf `live`-Branch umstellen (⚠️ vor erstem Test-Push, sonst geht main sofort live), Hook v4.11 an BEIDE Backends deployen, optional github.io aus Live-CORS entfernen. +tests/pure.test.mjs (pickApiUrl: Live-Host→Live, sonst→Test). 110 grün. app.js?v=43→44, admin-app.js?v=14→15, view-app.js?v=3→4, pb-login-bundle.js?v=1→2. BACKEND-SCHRITTE (Admin): Test-Instanz anlegen + Branch-Umstellung + Hook-Doppel-Deploy — siehe docs/umzug-zwei-umgebungen.md. NB: Deploy-Ablauf ändert sich — `git push origin main` = auf TEST veröffentlichen, Go-Live ist ein separater `main`→`live`-Push (siehe „Deploy"-Abschnitt unten).
v0.30.2 — fix(v0.30.1-Nachfix: Vormerken erzeugte falsche „Termin entfernt"-Absage): Code-Review (/code-review-excellence) des v0.30.1-`_applyPencil` (dropdown.js) fand: die neue „✎ Vorgemerkt: Name"-Auswahl rief bei JEDEM bestehenden Status-Record erst `_removeAssignment()`. Für einen bestätigten/angefragten Slot macht das einen Soft-Cancel + `_queueRemovedSlot()` → `{kind:'removed', changes:['Termin entfernt']}` in die Update-Queue, den der Self-Heal in `_updateCrewUpdateBar` bewusst NIE entfernt (userView.js:469, removed-Slots sind per Definition nicht mehr in getVal). Direkt danach patcht `pencilInAssignment` denselben Record via pbUpsert auf `pencilled` — die Person steht also weiter (vorgemerkt) in der Zelle, aber die Queue trägt fälschlich „Termin entfernt" → beim nächsten manuellen Update-Versand bekäme die Person eine falsche Absage-Mail. Trat nur im GLEICHE-PERSON-Fall auf (A im Slot → A vorgemerkt); der bestehende Quick-Toggle „✎ Vorläufig vormerken" (dropdown.js:150) hatte den Bug NIE, weil er `pencilInAssignment` direkt ohne `_removeAssignment` ruft. KLÄRUNG im Superpowers-Brainstorming (User): „vorgemerkt" ist KEINE Zurückstufung, sondern der neutrale Zukunfts-Planungsstatus (Termine/Besetzung noch nicht verbindlich) — nur eine Absage ist eine echte Herabstufung; UND: eine verdrängte ANDERE Person muss zwar benachrichtigt werden, aber wie überall NICHT automatisch, sondern per Knopfdruck (= genau das Queue-Modell). FIX (nur dropdown.js): `_removeAssignment` in `_applyPencil` nur noch bei Verdrängung einer ANDEREN aktiven Person aufrufen — `if(si && si.crewName && !sameCrew(si.crewName,name))`. Damit: A→A vorgemerkt = kein Queue-Eintrag/keine falsche Mail (wie Quick-Toggle); A(aktiv)→B vorgemerkt = A's Entfernung landet in der Queue (Versand erst per „Updates senden"). `sameCrew` war bereits importiert; `pencilInAssignment` überschreibt den einen Zell-Record (Filter plan_id&&date&&pos_id) in-place, ein vorheriges Entfernen ist für die reine Zuordnung unnötig. 108 Tests grün. Kein Backend/Schema, kein eigener Cache-Bust (dropdown.js hängt an app.js).
v0.30.1 — fix(„Vorgemerkt" im Zellen-Menü nicht erreichbar): User-Report: der v0.29.0-Status „Vorgemerkt" (✎, kein Mailversand) war im Zellen-Dropdown (`openCrewDD`, dropdown.js) praktisch unauffindbar — der Quick-Toggle „✎ Vorläufig vormerken" erschien NUR, wenn die Zelle schon eine Person zeigte (Standard-Crew/Override) UND noch KEIN Status-Record existierte (Gate `planned && !si`); bei leerer Zelle («— Nicht besetzt») fehlte die Option komplett (erst Person wählen → Menü erneut öffnen), bei bereits angefragtem/bestätigtem/abgelehntem Slot verschwand sie ganz (kein Zurücksetzen möglich). User wollte „Vorgemerkt" als normale Auswahl direkt im Menü, wie die anderen Optionen auch. FIX (nur dropdown.js, kein Backend/Schema): (1) Quick-Toggle-Gate von `planned && !si` auf `planned` verkürzt — funktioniert jetzt auch bei bestehendem Status (Zurücksetzen auf vorgemerkt möglich). (2) NEUE Auswahl-Sektion in der Crew-Liste: pro Person zusätzlich ein Eintrag „✎ Vorgemerkt: {Name}" (`_applyPencil`, ruft `pencilInAssignment` + `setAssign`), funktioniert für JEDE Zelle (leer oder belegt) und JEDE Crew-Person direkt, ohne Zwischenschritt. 108 Tests weiterhin grün (kein Testpfad berührt). Kein Cache-Bust-Version-Bump nötig (dropdown.js hat keine eigene ?v=-Query, hängt an app.js).
v0.30.0 — feat(Vereinheitlichte Änderungs-Mail + Aktivitäts-Log): User-Wunsch nach dem v0.29.2/3-Verlauf — „Update" heißt: JEDE Änderung an bestätigten Terminen, egal ob ein Tag HINZUKOMMT oder WEGFÄLLT, gehört in EINE „Es gab Änderungen"-Mail mit dynamischem Inhalt (neue Termine: „bestätige, dass du Zeit hast"; entfernte: „bestätige, dass du die Änderung gesehen hast"). Design-Spec docs/superpowers/specs/2026-07-21-unified-change-mail-design.md (Ansatz A, approved), Plan docs/superpowers/plans/2026-07-21-unified-change-mail.md, Umsetzung via Subagent-Driven-Development (pro Task frischer Implementer + Spec-Review + Quality-Review). KERNENTSCHEIDUNG „Soft-Cancel": Beim Entfernen einer bestätigten/angefragten Person wird der assignments-Record NICHT mehr gelöscht, sondern auf `status:'cancelled'` gepatcht — nur so hat der „ÄNDERUNGEN GESEHEN ✓"-Mail-Button ein Ziel (aid). Quittung → `cancel_acked`. Beide Werte werden aus ALLEN Zellen-Ladepfaden gefiltert (loadAssignmentStatuses + view-app.js) → Tabelle zeigt sie nie; Wiederbesetzen desselben Slots überschreibt den cancelled-Record via bestehendem pbUpsert (deshalb Guard: Quittung patcht NUR noch-cancelled-Records + eigene E-Mail). UMSETZUNG: (1) dataService: `softCancelAssignment` (warn+rethrow-Muster), `ackCancelledAssignments(aids)→n` (Status+Email-Guard, leere-Email-Guard), `logActivity(action,{...})` (fire-and-forget POST auf NEUE Collection activity_log — Fehler brechen nie den Hauptflow). (2) userView-Queue: Slots tragen `kind:'new'|'removed'` (fehlend=legacy=new); NEU `_queueRemovedSlot(...aid)`; Self-Heal-Ausnahme für removed (die sind per Definition nicht mehr in getVal — OHNE die Ausnahme würde der Self-Heal sie sofort wieder wegheilen!); Modal zeigt ➕ neu/➖ entfernt-Tags. (3) dropdown.js: `_notifyIfWasActive`→async `_removeAssignment` in allen 4 Entfernen-/Ersetzen-Pfaden (confirmed/proposed→Soft-Cancel+Queue; pencilled/declined→hartes Löschen wie bisher, die brauchen keine Absage-Info). (4) Das rote Absage-Banner unten rechts (v0.29.2) ist ERSETZT und entfernt (index.html+app.js+crewNotify.js; das manuelle Konsolen-„Absage"-Werkzeug v0.29.3 bleibt als Nachtrag-Weg). (5) `_sendUpdateForEntry` trennt removed/normal; removed gehen mit {kind,aid} in die Mail und werden NICHT auf proposed gepatcht; WICHTIGER Guard `&& normal.length` beim Fallback — ein rein-entfernter Eintrag darf nicht versehentlich den ganzen Plan anfragen. (6) HOOK v4.10 (Goja: nur var, kein Template-Literal, Helfer im Handler): `type==='update'` rendert „Es gab Änderungen." mit ➕-Abschnitt (TERMINE BESTÄTIGEN→App) und ➖-Abschnitt mit Button `?action=ackcancel&aids=id1,id2` (nur wenn aids vorhanden); rückwärtskompatibel (Slots ohne kind=neu). (7) authService `_handleEmailAction`: neuer `ackcancel`-Zweig (im try, differenzierter Toast, Resync) + logActivity bei Mail-confirm/decline; dataService confirm/decline loggen In-App-Reaktionen NUR bei `IS_CREW && !IS_MANAGER` (Manager-Klicks sind keine Crew-Reaktion — Mail-Links loggen immer, da per crew_email-Guard nur die Person selbst klicken kann). (8) NEUE PB-Collection **activity_log** { plan_id, crew_name, crew_email, action, date, pos_label, ts } — 2026-07-21 per Superuser live angelegt + mit Test-Record verifiziert; `ts` CLIENT-gesetzt (Collections haben kein created-Feld, sort=-id-Gotcha); Rules auth!="" (delete superadmin), ⚠️ Redeploy/Reimport-Caveat wie üblich. (9) admin.html: neuer „Aktivität"-Tab (renderActivityLog, perPage=200 sort=-id) + Login-Popup checkNewActivity (Zeilen neuer als localStorage `tourplan_activity_last_seen`; Timestamp wird VOR dem Early-Return gesetzt → Popup zeigt dieselben Zeilen nie doppelt); alle dynamischen Werte esc()-escaped. TESTS: +3 (softCancel patcht statt löscht + Cache-Leerung; ack nur cancelled + activity_log-POST; removed-Slots überleben Self-Heal) → 108 grün (TZ UTC+Berlin). VERIFIZIERT echt im Browser (echtes state/dropdown/dataService, fetch-Mock): „— Nicht besetzt" bei bestätigter Person → PATCH status:cancelled (kein DELETE) + Queue-Eintrag kind:removed mit aid. app.js?v=42→43, admin-app.js?v=13→14. ROLLOUT abgeschlossen: Hook v4.10 am 2026-07-22 vom Admin deployt (Log bestätigt, `/api/health`→200), activity_log-Collection war schon vorher live.
...
**Cache-Bust-Stand:** `index.html` lädt `theme.css?v=2` + `styles.css?v=25` + `js/app.js?v=45`; `view.html` lädt `js/view-app.js?v=4`; `admin.html` lädt `js/admin-app.js?v=15`; `login.html` lädt `js/pb-login-bundle.js?v=2`. Alle 4 Seiten laden `theme.css?v=2` (zentrale Token-/Font-Ebene, seit v0.28.0; helle Palette „Sage" seit v0.28.1). **Seit v0.19.0 hält der Service Worker (`sw.js`) alle Module dauerhaft frisch** (network-first/`no-cache`) → normalerweise KEIN Hard-Reload mehr nötig. Beim ERSTEN Laden nach dem v0.19.0-Deploy installiert sich der SW (einmaliger Auto-Reload via `controllerchange`, sonst 1× Hard-Reload). Danach kommen Updates automatisch an. Fällt der SW aus (kein SW-Support/deregistriert): Sub-Module laden ohne `?v=` → dann wie früher Hard-Reload. Kill-Switch: Kommentar in `sw.js`.

---

## Deploy: Zwei Umgebungen (seit v0.31.0)

**`main` = TEST, `live` = PRODUKTION.** Ein `main`-Push geht NICHT mehr live!

**Auf Test veröffentlichen** (GitHub Pages `aniflu.github.io/Crewplaner`, Test-PocketBase):

```bash
git push origin main
```

GitHub Pages aktualisiert sich ~1 Min nach dem Push. Hier gefahrlos testen (eigene Test-DB, kein Mailversand).

**Go-Live** (Hetzner-Container `crewplanner.nyxlightwork.de`, Live-PocketBase):

```bash
git checkout live && git merge main && git push origin live && git checkout main
```

Coolify baut den Live-Container automatisch aus dem `live`-Branch neu (~1–2 Min). Kein SSH/Admin nötig.

- Das Frontend wählt seine PocketBase **automatisch nach Hostname** (`pickApiUrl`, js/pure.js) — derselbe Code läuft in beiden Umgebungen, nichts umschreiben.
- Server-Setup der beiden Umgebungen (Test-Backend anlegen, Live-Branch-Umstellung, Hook-Deploy an beide Backends): **docs/umzug-zwei-umgebungen.md** (Admin-Anleitung).
- Hook-Deploys gehen ab jetzt an **beide** PocketBase-Instanzen (erst Test prüfen, dann Live) — siehe die Anleitung.
...
Aktuell deployte Hook-Version: **v4.11** — ⚠️ **v4.12 liegt im Repo und ist NOCH NICHT deployt** (v0.50.0: Statuswechsel-Abschnitt in der Update-Mail + vorgemerkte Termine im Kalender-Feed). Bis zum Deploy läuft die App vollständig; nur Mail-Wortlaut und Kalender-Status hinken hinterher. Bisheriger Stand: v4.11 — auf **BEIDEN** Backends (Live + Test) deployt 2026-07-30 durch Admin (Log `v4.11 geladen` + `/api/health` → 200 auf beiden bestätigt). v4.11 = RESEND_KEY-Guard (Test ohne Key sendet keine Mails) + Doc-Links auf `crewplanner.nyxlightwork.de/docs`. Feld `users.feed_token` ist angelegt + alle 10 User backfilled (2026-07-19 per Superuser). Kalender-Abo **tour-spezifisch** (`/ics/{token}/{plan}`) end-to-end verifiziert: Peter Weist getrennt (AMK 2026 = 53 CONFIRMED, AMK 2027 = 33 TENTATIVE, keine Vermischung), bare `/ics/{token}` → 404.
...
- v4.12: feat (v0.50.0, **NOCH NICHT deployt — muss auf BEIDE Backends, erst Test, dann Live**) — (a) `/ics/{token}/{plan}` nimmt jetzt auch `status = "pencilled"` mit auf; der Tagesstatus kommt aus einer RANK-Tabelle (`confirmed > proposed > pencilled`) statt des bisherigen bool-Flags; die `DESCRIPTION` trägt eine Zeile `Status: Bestätigt|Angefragt|Vorgemerkt`, `STATUS:CONFIRMED` nur noch bei bestätigt. Die stabile UID bleibt → Kalender-Apps ERSETZEN den Termin beim nächsten Abruf, der Status wandert ohne Dublette mit. Vorher fielen vorgemerkte Termine aus dem Feed und verschwanden lautlos aus dem Kalender der Person. (b) `type==='update'` rendert zwei neue Abschnitte für `kind==='status'`-Slots: `to==='pencilled'` → „✎ Jetzt vorgemerkt — vorläufig geplant, noch nicht verbindlich. Du musst nichts tun.“; `to==='confirmed'` → „✓ Wieder verbindlich bestätigt.“ Bewusst OHNE Aktions-Button (nichts zu quittieren). Rückwärtskompatibel: unter v4.11 landen diese Slots im generischen ➕-Abschnitt. Kein Schema-Schritt.
...
Assignment-Status-Werte: `proposed` → `confirmed` | `declined` | `pencilled` (v0.29.0, „vorgemerkt"; seit v0.50.0 auch per Sammel-Dialog aus `confirmed` heraus und zurück) | `cancelled` → `cancel_acked` (v0.30.0, Soft-Cancel statt Löschen)
