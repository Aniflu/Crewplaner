...
- **45 Tests grün** (Stand v0.17.1). Test-Dateien (`tests/*.test.mjs`, auto-discovered von `run.mjs`):
  ...
  - **Guards** (fangen ganze Fehlerklassen statisch):
    ...
    - `queue.test.mjs` — `_queueGlobalCrewUpdate(desc, dates)` queued nur Slots der übergebenen Datumswerte (kein Plan-Flut bei Datum-Hinzufügen).
...
**Cache-Bust-Stand:** `index.html` lädt `js/app.js?v=21`, `admin.html` lädt `js/admin-app.js?v=4`. Sub-Module laden OHNE `?v=` → bei „Crew/Wolf sieht nichts" zuerst Hard-Reload/Cache-Clear (stale Sub-Modul bricht den Graphen). Per-Modul-Versionsnummern werden nicht mehr gepflegt.
